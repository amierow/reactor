class RatingsController < ApplicationController
  def index
    ...
    @ratings = @q.result(:distinct => true).includes(:contributor, :question, :rating_type, :presentation).page(params[:page]).per(10)
  ...
  def create
    ...
    @rating.insight_id = params[:insight_id]
    @rating.contributor_id = params[:contributor_id]
    @rating.rating_type_id = params[:rating_type_id]
    @rating.rating = params[:rating]
    @rating.feedback = params[:feedback]
